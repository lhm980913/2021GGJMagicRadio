...
Shader "Unlit/outline"
{
	...
	Properties
	{
		...
		_EdgeAlphaThreshold("Edge Alpha Threshold", range(-1,3)) = 1.0		//边界透明度的阈值
		_EdgeColor("Edge Color", Color) = (0,0,0,1)				//边界的颜色
		_EdgeDampRate("Edge Damp Rate", range(0,1)) = 2				//渐变的分母
		...
		SubShader
		{
			...
			Pass
			{
				...
				fixed4 frag(v2f i) : SV_Target
				{
					...
					float a = _EdgeAlphaThreshold + (sin(_Time.z*3)+1)/2;
					
						half alphaSum = CalculateAlphaSumAround(i);
						float isNeedShow = alphaSum > a;
						float damp = saturate((alphaSum - a) * _EdgeDampRate);
						...
						//2D图片的内发光
							...
							//计算透明度的和
							float alphaCircleSum = CalculateCircleSumAlpha(i.uv[4], _InnerGlowWidth / 100, _InnerGlowAccuracy) / _InnerGlowAccuracy;
